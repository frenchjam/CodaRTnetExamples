...
namespace codaRTNet
{
	/** Summary
		...
		*/
	class DeviceOptionsTeleMyo : public DeviceOptions
	{
	public:

		/** Underlying device options structure */
		CODANET_DEVICEOPTIONS_TELEMYO_INPUTS opt;
		...
		/** Summary
			...
			*/
		DeviceOptionsTeleMyo(float fBaseRateHz)
		{
			opt.base.dwSize = sizeof(opt);
			opt.base.dwCommand = CODANET_SETDEVICEOPTIONS;
			opt.base.wDevice = DEVICEID_TELEMYO;
			opt.base.wOptionID = CODANET_DEVICEOPTIONID_TELEMYO_INPUTS;
			opt.fBaseRateHz = fBaseRateHz;
			for (DWORD i = 0; i < CODANET_TELEMYO_MAX_CHANNELS; i++)
	...
	protected:
		...
		{ codanet_deviceoptions_telemyo_inputs_hton((CODANET_DEVICEOPTIONS_TELEMYO_INPUTS*)data); }

		virtual void ntoh(void* data) const
		{ codanet_deviceoptions_telemyo_inputs_ntoh((CODANET_DEVICEOPTIONS_TELEMYO_INPUTS*)data); }
	};
